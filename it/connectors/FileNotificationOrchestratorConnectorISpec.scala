...
class FileNotificationOrchestratorConnectorISpec extends IntegrationSpecCommonBase {
  ...
  "getFileNotificationOrchestrator" should {
    "call file notification orchestrator and handle a successful response" in new Setup {
      val result: HttpResponse = await(connector.getFileNotificationOrchestrator())
