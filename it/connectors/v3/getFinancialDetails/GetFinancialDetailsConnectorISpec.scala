...
class GetFinancialDetailsConnectorISpec extends IntegrationSpecCommonBase with ETMPWiremock with FeatureSwitching {
  ...
  "getFinancialDetails" should {
    "return a successful response when called" in new Setup {
      ...
      val result: GetFinancialDetailsResponse = await(connector.getFinancialDetails("/VRN/123456789/VATC", LocalDate.now(), LocalDate.now())(hc))
    ...
    s"return a $GetFinancialDetailsMalformed response when called" in new Setup {
      ...
      val result: GetFinancialDetailsResponse = await(connector.getFinancialDetails("/VRN/123456789/VATC", LocalDate.now(), LocalDate.now()))
