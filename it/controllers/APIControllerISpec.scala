...
class APIControllerISpec extends IntegrationSpecCommonBase with ETMPWiremock with FeatureSwitching {
 ...
 "getFinancialDetails" should {
   val fullQueries = "?searchType=CHGREF&searchItem=XC00178236592&dateType=BILLING&dateFrom=2020-10-03&dateTo=2021-07-12" +
   ...
   "return an OK status" when {
     "the get Financial Details call succeeds with no extra query parameters" in {
       val sampleAPI1811Response = Json.parse(
         """
           ...
           |}
           |}""".stripMargin)
       mockResponseForGetFinancialDetails(Status.OK, s"VRN/123456789/VATC", Some(getFinancialDetailsAsJson.toString()))
       val result = await(buildClientForRequestToApp(uri = s"/penalty/financial-data/VRN/123456789/VATC").get())
       result.status shouldBe OK
     ...
     "the get Financial Details call succeeds with full query parameters" in {
       val sampleAPI1811Response = Json.parse(
         """
           |{
           | "getFinancialData" : {
           ...
           |}
           |}""".stripMargin)
       mockResponseForGetFinancialDetails(Status.OK, s"VRN/123456789/VATC$fullQueries", Some(getFinancialDetailsAsJson.toString()))
       val result = await(buildClientForRequestToApp(uri = s"/penalty/financial-data/VRN/123456789/VATC$fullQueries").get())
       result.status shouldBe OK
       result.json shouldBe sampleAPI1811Response
       wireMockServer.findAll(postRequestedFor(urlEqualTo("/write/audit"))).asScala.toList
         .exists(_.getBodyAsString.contains("Penalties3rdPartyFinancialPenaltyDetailsDataRetrieval")) shouldBe true
   ...
   "return the status from EIS" when {
     "404 response received " in {
       mockResponseForGetFinancialDetails(Status.NOT_FOUND, s"VRN/123456789/VATC$fullQueries")
       val result = await(buildClientForRequestToApp(uri = s"/penalty/financial-data/VRN/123456789/VATC$fullQueries").get())
       result.status shouldBe NOT_FOUND
       wireMockServer.findAll(postRequestedFor(urlEqualTo("/write/audit"))).asScala.toList
         .exists(_.getBodyAsString.contains("Penalties3rdPartyFinancialPenaltyDetailsDataRetrieval")) shouldBe true
     ...
     "Non 200 response received " in {
       mockResponseForGetFinancialDetails(Status.BAD_REQUEST, s"VRN/123456789/VATC$fullQueries", Some(""))
       val result = await(buildClientForRequestToApp(uri = s"/penalty/financial-data/VRN/123456789/VATC$fullQueries").get())
       result.status shouldBe BAD_REQUEST
       wireMockServer.findAll(postRequestedFor(urlEqualTo("/write/audit"))).asScala.toList
         .exists(_.getBodyAsString.contains("Penalties3rdPartyFinancialPenaltyDetailsDataRetrieval")) shouldBe true
