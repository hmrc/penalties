...
class AppealsControllerISpec extends IntegrationSpecCommonBase with ETMPWiremock with AppealWiremock {
  ...
  "getAppealsDataForLateSubmissionPenalty" should {
    ...
    "call ETMP and compare the penalty ID provided and the penalty ID in the payload - returning the oldest VAT period if there is multiple periods present" in {
  ...
  "submitAppeal" should {
    "call the connector and send the appeal data received in the request body - returns OK when successful for bereavement" in {
      ...
      val result = await(buildClientForRequestToAppWithCorrelationId(uri = "/appeals/submit-appeal?enrolmentKey=HMRC-MTD-VAT~VRN~123456789&isLPP=false&penaltyId=123456789").post(
        jsonToSubmit
      ))
      result.status shouldBe OK
    }
      ...
      "call the connector and send the appeal data received in the request body - returns OK when successful for LPP" in {
        ...
        val jsonToSubmit: JsValue = Json.parse(
          """
            ...
            |}
            |""".stripMargin)
        val result = await(buildClientForRequestToAppWithCorrelationId(uri = "/appeals/submit-appeal?enrolmentKey=HMRC-MTD-VAT~VRN~123456789&isLPP=true&penaltyId=123456789").post(
          jsonToSubmit
        ))
        result.status shouldBe OK
      }
    ...
    "return BAD_REQUEST (400)" when {
      "no JSON body is in the request" in {
        val result = await(buildClientForRequestToAppWithCorrelationId(uri = "/appeals/submit-appeal?enrolmentKey=HMRC-MTD-VAT~VRN~123456789").post(
          ""
        ))
  ...
  "getIsMultiplePenaltiesInSamePeriod" should {
    ...
    "return OK" when {
      "the service returns true" in {
        ...
        val result = await(buildClientForRequestToAppWithCorrelationId(uri =
