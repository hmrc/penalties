...
class PenaltiesFrontendControllerISpec extends IntegrationSpecCommonBase with HipFinancialWiremock
    ...
    Table(
      ...
      s"return OK (${Status.OK}) for $regime when using HIP API" when {
        "the get HIP penalty details call succeeds and the get financial details call succeeds" in {
            ...
            mockResponseForGetPenaltyDetails(Status.OK, regime, idType, id, responseBody = Some(hipPenaltyDetailsJson.toString()))
      ...
      s"return ISE (${Status.INTERNAL_SERVER_ERROR}) for $regime when using HIP API" when {
        "the get HIP penalty details call fails" in {
            ...
            mockStubResponseForAuthorisedUser
            mockResponseForGetPenaltyDetails(Status.INTERNAL_SERVER_ERROR, regime, idType, id, responseBody = Some(""))
        ...
        "the get HIP penalty details call returns 422 without incorrect ID number message" in {
            ...
            mockStubResponseForAuthorisedUser
            mockResponseForGetPenaltyDetails(Status.UNPROCESSABLE_ENTITY, regime, idType, id,
              responseBody = Some("""{"errors":{"processingDate":"2025-03-03"}}"""))
      ...
      s"return NOT_FOUND (${Status.NOT_FOUND}) for $regime when using HIP API" when {
        "the get HIP penalty details call returns 404" in {
            ...
            mockStubResponseForAuthorisedUser
            mockResponseForGetPenaltyDetails(Status.NOT_FOUND, regime, idType, id,
              responseBody = Some("""{"response":{"error":{"code":"404","message":"NOT_FOUND","logId":"errorLogId"}}}"""))
