...
trait RegimeETMPWiremock {
  ...
  val getFinancialDetailsAsJsonIF: JsValue = Json.parse(
    """
      |{
      |"getFinancialData": {
      |   "financialDetails":{
  ...
  def mockStubResponseForGetFinancialDetailsIF(regime: Regime,
                                               idType: IdType,
                                               id: Id, status: Int,
                                               queryParams: Option[String] = None,
                                               responseBody: Option[String] = None): StubMapping = {
    stubFor(get(urlEqualTo(s"/penalty/financial-data/${idType.value}/${id.value}/${regime.value}${queryParams.getOrElse("")}"))
      .willReturn(
        aResponse()
          .withBody(responseBody.fold(getPenaltyDetailsWithLSPAndLPPAsJson.toString())(identity))
          .withStatus(status)
      ))
  }

  def mockStubResponseForGetFinancialDetails(status: Int, responseBody: Option[String] = None): StubMapping = {
