...
class AppealsControllerSpec extends SpecBase with FeatureSwitching with LogCapturing {
  ...
  val sampleSDESNotifications: Seq[SDESNotification] = Seq(
    SDESNotification(
      ...
      file = SDESNotificationFile(
        ...
        size = 987,
        properties = Seq(
          SDESProperties("CaseId", "PR-123456789"),
          SDESProperties("SourceFileUploadDate", "2018-04-24T09:30:00Z")
        )
      ),
  ...
  class Setup(withRealAppConfig: Boolean = true) {
    ...
    val controller = new AppealsController(
      if (withRealAppConfig) appConfig else mockAppConfig,
      mockAppealsService,
    ...
    "getAppealsDataForLateSubmissionPenalty" should {
      ...
      s"return NOT_FOUND (${Status.NOT_FOUND}) when ETMP can not find the data for the given enrolment key" in new Setup {
        ...
        when(mockGetPenaltyDetailsService.getDataFromPenaltyServiceForVATCVRN(ArgumentMatchers.eq(vrn))(ArgumentMatchers.any()))
    ...
    "getAppealsDataForLatePaymentPenalty" should {
      ...
      s"return NOT_FOUND (${Status.NOT_FOUND}) when ETMP returns data but the given penaltyId is wrong" in new Setup {
        ...
        val result: Future[Result] =
          controller.getAppealsDataForLatePaymentPenalty(samplePenaltyId, sampleEnrolmentKey, isAdditional = false)(fakeRequest)
        status(result) shouldBe Status.NOT_FOUND
        contentAsString(result) shouldBe "Penalty ID was not found in users penalties."
      }
      ...
      s"return ISE (${Status.INTERNAL_SERVER_ERROR}) when API 1812 call returns malformed data" in new Setup {
        ...
        withCaptureOfLoggingFrom(logger) { logs =>
          val result: Future[Result] =
            controller.getAppealsDataForLatePaymentPenalty("1234567891", sampleEnrolmentKey, isAdditional = false)(fakeRequest)
          status(result) shouldBe Status.INTERNAL_SERVER_ERROR
          logs.exists(_.getMessage.contains(PagerDutyKeys.MALFORMED_RESPONSE_FROM_1812_API.toString)) shouldBe true
        }
      }
      ...
      s"return OK (${Status.OK}) when the call to ETMP succeeds and the penalty ID matches for Additional penalty" in new Setup {
        ...
        val result: Future[Result] =
          controller.getAppealsDataForLatePaymentPenalty(samplePenaltyId, sampleEnrolmentKey, isAdditional = true)(fakeRequest)
        status(result) shouldBe Status.OK
        val appealDataToReturn: AppealData = AppealData(
          `type` = Additional,
      ...
      "return BAD_REQUEST (400)" when {
        "the request body is not valid JSON" in new Setup {
          val result: Future[Result] = controller.submitAppeal(
            "HMRC-MTD-VAT~VRN~123456789",
            isLPP = false,
            penaltyNumber = "123456789",
            correlationId = correlationId,
            isMultiAppeal = false)(fakeRequest)
        ...
        "the request body is valid JSON but can not be serialised to a model" in new Setup {
          ...
          val result: Future[Result] = controller.submitAppeal(
            ...
            penaltyNumber = "123456789",
            correlationId = correlationId,
            isMultiAppeal = false)(fakeRequest.withJsonBody(appealsJson))
          status(result) shouldBe BAD_REQUEST
      ...
      "when the appeal is part of a multi appeal" should {
          ...
          withCaptureOfLoggingFrom(logger) { logs =>
            val result: Result = await(
              controller.submitAppeal(
                ...
                penaltyNumber = "123456789",
                correlationId = correlationId,
                isMultiAppeal = true)(fakeRequest.withJsonBody(appealsJson)))
        ...
        "return 207 (MULTI_STATUS) if the file notification call fails (with exception) and audit the storage failure" in new Setup {
          ...
          when(mockAppealsService.createSDESNotifications(any(), any()))
            .thenReturn(sampleSDESNotifications)
          ...
          withCaptureOfLoggingFrom(logger) { logs =>
            ...
            val expectedLogMessage =
              s"[AppealsController][submitAppealToPEGA] Unable to store file notification for user with enrolment: HMRC-MTD-VAT~VRN~123456789 penalty 123456789 (correlation ID: $correlationId) - An unknown exception occurred when attempting to store file notifications, with error: failed"

            logs.exists(log => log.getMessage.contains(expectedLogMessage)) shouldBe true
    ...
    "AppealsController.getMultiplePenaltyData" should {
      val sampleEnrolmentKey: String = "HMRC-MTD-VAT~VRN~123456789"
      val penaltyId: String          = "1234567891"
      val vrn: String                = "123456789"

      "return ISE (500) when API 1812 call returns 500 error and logs RETRIEVE_PENALTY_REFORM_500" in new Setup {
        when(mockGetPenaltyDetailsService.getDataFromPenaltyServiceForVATCVRN(ArgumentMatchers.eq(vrn))(ArgumentMatchers.any()))
          .thenReturn(Future.successful(Left(GetPenaltyDetailsFailureResponse(Status.INTERNAL_SERVER_ERROR))))

        withCaptureOfLoggingFrom(logger) { logs =>
          val result: Future[Result] = controller.getMultiplePenaltyData(penaltyId, sampleEnrolmentKey)(fakeRequest)
          status(result) shouldBe Status.INTERNAL_SERVER_ERROR
          logs.exists(_.getMessage.contains(RETRIEVE_PENALTY_REFORM_500.toString)) shouldBe true
        }
      }

      "return BadRequest (400) when API 1812 call returns 400 error and logs RETRIEVE_PENALTY_REFORM_REQUEST_FAILED" in new Setup {
        when(mockGetPenaltyDetailsService.getDataFromPenaltyServiceForVATCVRN(ArgumentMatchers.eq(vrn))(ArgumentMatchers.any()))
        ...
        withCaptureOfLoggingFrom(logger) { logs =>
          ...
          logs.exists(_.getMessage.contains(RETRIEVE_PENALTY_REFORM_REQUEST_FAILED.toString)) shouldBe true
        }
      }

      "return ISE (500) when API 1812 call returns malformed data and logs MALFORMED_RESPONSE_FROM_1812_API" in new Setup {
        withCaptureOfLoggingFrom(logger) { logs =>
          val result: Future[Result] = controller.getMultiplePenaltyData(penaltyId, sampleEnrolmentKey)(fakeRequest)
          status(result) shouldBe Status.INTERNAL_SERVER_ERROR
          logs.exists(_.getMessage.contains(MALFORMED_RESPONSE_FROM_1812_API.toString)) shouldBe true
        }
      }
    }
  }
