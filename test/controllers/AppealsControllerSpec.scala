  ...
  "submitAppeal" should {
    "return BAD_REQUEST (400)" when {
      "the request body is not valid JSON" in new Setup {
        val result = controller.submitAppeal("HMRC-MTD-VAT~VRN~123456789")(fakeRequest)
      ...
      "the request body is valid JSON but can not be serialised to a model" in new Setup {
        ...
        val result = controller.submitAppeal("HMRC-MTD-VAT~VRN~123456789")(fakeRequest.withJsonBody(appealsJson))
        status(result) shouldBe BAD_REQUEST
