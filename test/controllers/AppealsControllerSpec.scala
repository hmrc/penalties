      ...
      "when the appeal is part of a multi appeal" should {
        ...
        "return 207 (MULTI_STATUS) if the file notification call fails (with exception) and audit the storage failure" in new Setup {
          ...
          withCaptureOfLoggingFrom(logger) { logs =>
            ...
            result.header.status shouldBe MULTI_STATUS
            contentAsJson(Future(result)) shouldBe expectedJsonResponse
            val expectedLogMessage = s"[AppealsController][submitAppealToPEGA] Unable to store file notification for user with enrolment: HMRC-MTD-VAT~VRN~123456789 penalty 123456789 (correlation ID: $correlationId) - An unknown exception occurred when attempting to store file notifications, with error: failed"
