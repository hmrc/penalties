...
class PenaltiesFrontendControllerSpec extends SpecBase with LogCapturing {
  ...
  class Setup(isFSEnabled: Boolean = true) {
    ...
    val controller: PenaltiesFrontendController = new PenaltiesFrontendController(
      ...
      dateHelper,
      cc
    )(implicitly, mockConfig, authComponent)
  }
    ...
    "use API 1812 data and combine with API 1811 data" must {
      ...
      "return UNAUTHORIZED (401)" when {
        "the caller does not provide an auth token" in new Setup {
          val result: Future[Result] = controller.getPenaltiesData("123456789", Some(""))(FakeRequest("GET", "/"))
      ...
      "return FORBIDDEN (403)" when {
        "the caller does not have the sufficient permissions" in new Setup {
          ...
          val result: Future[Result] = controller.getPenaltiesData("123456789", Some(""))(fakeRequest)
