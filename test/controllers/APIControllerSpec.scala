...
class APIControllerSpec extends SpecBase with FeatureSwitching with LogCapturing {
  ...
  "getFinancialDetails" should {
    s"return OK (${Status.OK}) when a JSON payload is received from EIS (auditing the response)" when {
      Seq(OK, CREATED).foreach { responseStatus =>
        s"response status is $responseStatus" in new Setup {
  ...
  "getPenaltyDetails" should {
    s"return OK (${Status.OK}) when a valid JSON payload is returned from the connector (auditing the response)" in new Setup {
  ...
  "getSummaryData with unified API" should {
    ...
    s"return OK (${Status.OK}) when unified service returns penalty data" in new Setup {
      ...
      when(mockGetFinancialDetailsService
        .getFinancialDetails(any(), any())(any()))
        .thenReturn(Future.successful(Right(FinancialDetailsSuccessResponse(financialDetails))))
