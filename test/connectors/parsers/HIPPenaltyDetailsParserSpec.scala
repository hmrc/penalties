  ...
  "HIPPenaltyDetailsReads" should {
    s"parse an OK (${Status.OK}) response" when {
      s"the body of the response is valid" in {
        val result = HIPPenaltyDetailsParser.HIPPenaltyDetailsReads.read("GET", "/", httpResponse(mockHIPPenaltyDetailsModelv3))
      ...
      s"the body is malformed - returning a $Left $HIPPenaltyDetailsMalformed" in {
        val result = HIPPenaltyDetailsParser.HIPPenaltyDetailsReads.read("GET", "/", mockOKHttpResponseWithInvalidBody)
      ...
      "there is a posted LPP1 with a missing clearing date with associated LPP2" in {
        val result = HIPPenaltyDetailsParser.HIPPenaltyDetailsReads.read("GET", "/", httpResponse(mockHIPPenaltyDetailsModelWithMissingClearingDateForOnePostedLPP1))
      ...
      "there is a posted LPP1 with a missing clearing date without an associated LPP2 to retrieve the date from" in {
        val result = HIPPenaltyDetailsParser.HIPPenaltyDetailsReads.read("GET", "/", httpResponse(mockHIPPenaltyDetailsModelWithMissingClearingDateForOnePostedLPP1NoLPP2))
      ...
      "there are multiple posted LPP1s with missing clearing dates" in {
        val result = HIPPenaltyDetailsParser.HIPPenaltyDetailsReads.read("GET", "/", httpResponse(mockHIPPenaltyDetailsModelWithMissingClearingDateForMultiplePostedLPP1s))
      ...
      "there is an LPP1 with Accruing status and missing clearing date (should NOT be updated)" in {
        val result = HIPPenaltyDetailsParser.HIPPenaltyDetailsReads.read("GET", "/", httpResponse(mockHIPPenaltyDetailsModelWithAccruingLPP1))
    ...
    s"parse a BAD REQUEST (${Status.BAD_REQUEST}) response" in {
      val result = HIPPenaltyDetailsParser.HIPPenaltyDetailsReads.read("GET", "/", mockBadRequestHttpResponse)
      result.isLeft shouldBe true
    ...
    s"parse a NOT FOUND (${Status.NOT_FOUND}) response with JSON body returned (return $HIPPenaltyDetailsNoContent)" in {
      val result = HIPPenaltyDetailsParser.HIPPenaltyDetailsReads.read("GET", "/", mockNotFoundHttpResponseJsonBody)
      result.isLeft shouldBe true
    ...
    s"parse a NOT FOUND (${Status.NOT_FOUND}) response with no body" in {
      val result = HIPPenaltyDetailsParser.HIPPenaltyDetailsReads.read("GET", "/", mockNotFoundHttpResponseNoBody)
      result.isLeft shouldBe true
      result.left.getOrElse(HIPPenaltyDetailsFailureResponse(IM_A_TEAPOT)).asInstanceOf[HIPPenaltyDetailsFailureResponse].status shouldBe Status.NOT_FOUND
    }
    ...
    s"parse a Conflict (${Status.CONFLICT}) response" in {
      val result = HIPPenaltyDetailsParser.HIPPenaltyDetailsReads.read("GET", "/", mockConflictHttpResponse)
      result.isLeft shouldBe true
      result.left.getOrElse(HIPPenaltyDetailsFailureResponse(IM_A_TEAPOT)).asInstanceOf[HIPPenaltyDetailsFailureResponse].status shouldBe Status.CONFLICT
    }

    s"parse an UNPROCESSABLE ENTITY (${Status.UNPROCESSABLE_ENTITY}) response" in {
      val result = HIPPenaltyDetailsParser.HIPPenaltyDetailsReads.read("GET", "/", mockUnprocessableEntityHttpResponse)
      result.isLeft shouldBe true
      result.left.getOrElse(HIPPenaltyDetailsFailureResponse(IM_A_TEAPOT)).asInstanceOf[HIPPenaltyDetailsFailureResponse].status shouldBe Status.UNPROCESSABLE_ENTITY
    }

    s"parse an INTERNAL SERVER ERROR (${Status.INTERNAL_SERVER_ERROR}) response" in {
      val result = HIPPenaltyDetailsParser.HIPPenaltyDetailsReads.read("GET", "/", mockISEHttpResponse)
      result.isLeft shouldBe true
    ...
    s"parse a SERVICE UNAVAILABLE (${Status.SERVICE_UNAVAILABLE}) response" in {
      val result = HIPPenaltyDetailsParser.HIPPenaltyDetailsReads.read("GET", "/", mockServiceUnavailableHttpResponse)
      result.isLeft shouldBe true
      result.left.getOrElse(HIPPenaltyDetailsFailureResponse(IM_A_TEAPOT)).asInstanceOf[HIPPenaltyDetailsFailureResponse].status shouldBe Status.SERVICE_UNAVAILABLE
    }
