...
class FinancialDetailsHIPSpec extends SpecBase {
  ...
  val model: FinancialDetailsHIP = FinancialDetailsHIP(
    processingDate = "2023-11-28T10:15:10Z",
    FinancialDetails(
  ...
  "be readable from JSON" in {
    val result = Json.fromJson(modelAsJson)(FinancialDetailsHIP.reads)
    result.isSuccess shouldBe true
    result.get shouldBe model
  }

  "be writable to JSON" in {
    val result = Json.toJson(model)(FinancialDetailsHIP.writes)
