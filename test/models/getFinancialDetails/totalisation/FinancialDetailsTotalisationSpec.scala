...
class FinancialDetailsTotalisationSpec extends SpecBase {
  val modelAsJson: JsValue = Json.parse(
    """
      ...
      |   "totalAccountNotYetDue": 54.32,
      |   "totalAccountCredit": 23.45,
      |   "totalAccountBalance": 12.34
      | }
      |}
      |""".stripMargin)
  ...
  "be readable from JSON" in {
    val result = Json.fromJson(modelAsJson)(FinancialDetailsTotalisation.reads)
    result.isSuccess shouldBe true
    result.get shouldBe model
  }

  "be writable to JSON" in {
    val result = Json.toJson(model)(FinancialDetailsTotalisation.writes)
