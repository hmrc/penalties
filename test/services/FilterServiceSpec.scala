...
import models.hipPenaltyDetails.appealInfo.AppealStatusEnum._
...
class FilterServiceSpec extends SpecBase with MockitoSugar {
  ...
  private val non9xStatusInfoType = AppealInformationType(Some(Under_Appeal), None, None)
  "doesAppealInformationHave9xAppealStatus" should {
    "return 'true'" when {
      AppealStatusEnum.ignoredStatuses.foreach { status =>
        s"appealInformation contains ${status.toString}" in {
          val appealInformationType                                 = AppealInformationType(Some(status), None, None)
          val appealInformation: Option[Seq[AppealInformationType]] = Some(Seq(appealInformationType, non9xStatusInfoType))

          doesAppealInformationHave9xAppealStatus(appealInformation) shouldBe true
        }
      }
    }
    ...
    "return 'false'" when {
      "appealInformation is None" in {
        val appealInformation: Option[Seq[AppealInformationType]] = None

        doesAppealInformationHave9xAppealStatus(appealInformation) shouldBe false
      }
      "appealInformation is Some(Seq.empty)" in {
        val appealInformation: Option[Seq[AppealInformationType]] = Some(Seq.empty[AppealInformationType])

        doesAppealInformationHave9xAppealStatus(appealInformation) shouldBe false
      }
      "appealInformation contains a sequence with no 91-94 appeal statuses" in {
        val appealInformation: Option[Seq[AppealInformationType]] = Some(Seq(non9xStatusInfoType, non9xStatusInfoType))
