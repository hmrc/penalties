...
class FilterService @Inject() ()(implicit appConfig: AppConfig) {
  ...
  private def prepareLateSubmissionPenaltiesAfterFilter(penaltiesDetails: GetPenaltyDetails, filteredLSPs: Seq[LSPDetails], noOfFilteredLSPs: Int)(
    ...
    if (filteredLSPs.nonEmpty && noOfFilteredLSPs > 0) {
      ...
      logger.info(
        s"[RegimeFilterService][prepareLateSubmissionPenaltiesAfterFilter] Filtering for [${loggingContext.callingClass}][${loggingContext.function}] - Filtered all LSPs from payload for ${loggingContext.enrolmentKey}")
      None
    }

  private def prepareLatePaymentPenaltiesAfterFilter(penaltiesDetails: GetPenaltyDetails,
                                                     filteredLPPs: Option[Seq[LPPDetails]],
                                                     noOfFilteredLPPs: Int)(implicit loggingContext: LoggingContext): Option[LatePaymentPenalty] =
  ...
  private def filterLPPWith9xAppealStatus(penaltiesDetails: GetPenaltyDetails): Option[Seq[LPPDetails]] = {
    ...
    lppDetails.map(latePaymentPenalties => latePaymentPenalties.filterNot(lpp => appealInformationHas9xAppealStatus(lpp.appealInformation)))
  }
