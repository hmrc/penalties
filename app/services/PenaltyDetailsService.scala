...
import utils.{LoggingContext, PenaltyDetailsConverter}

import javax.inject.Inject
import scala.concurrent.{ExecutionContext, Future}

class PenaltyDetailsService @Inject() (penaltyDetailsConnector: HIPPenaltyDetailsConnector, filterService: FilterService)(implicit
    ec: ExecutionContext,
  ...
  def getPenaltyDetails(enrolmentKey: AgnosticEnrolmentKey)(implicit hc: HeaderCarrier): Future[GetPenaltyDetailsResponse] = {
    ...
    penaltyDetailsConnector.getPenaltyDetails(enrolmentKey).map {
      _.fold(
        failure =>
          convertHIPFailureToRegular(failure, enrolmentKey, startOfLogMsg),
        success => {
