...
class AppealService @Inject() (hipAppealsConnector: HIPSubmitAppealConnector, appConfig: AppConfig, idGenerator: UUIDGenerator)(implicit
    ec: ExecutionContext) {

  private val regexToSanitiseFileName: String = "[\\\\\\/:*?<>|\"‘’“”]"

  def submitAppeal(appealSubmission: AppealSubmission, enrolmentKey: AgnosticEnrolmentKey, penaltyNumber: String, correlationId: String)(implicit
      headerCarrier: HeaderCarrier): Future[Either[AppealsParser.ErrorResponse, AppealResponseModel]] =
    hipAppealsConnector.submitAppeal(appealSubmission, penaltyNumber, correlationId).flatMap {
      _.fold(
        error => {
          logger.error(
