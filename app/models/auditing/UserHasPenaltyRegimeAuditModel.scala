...
case class UserHasPenaltyRegimeAuditModel(
  ...
  private val amountOfLspChargesPaid: Int = penaltyDetails.lateSubmissionPenalty
    .map(
      _.details.count(point =>
        point.chargeOutstandingAmount.contains(BigDecimal(0))
          ...
          && point.penaltyStatus.equals(LSPPenaltyStatusEnum.Active)))
    .getOrElse(0)

  private val lspsUnpaidAndUnappealed: Seq[LSPDetails] = penaltyDetails.lateSubmissionPenalty
    .map(
      _.details.filter(point =>
        !point.chargeOutstandingAmount.contains(BigDecimal(0))
  ...
  private val amountOfLspChargesUnpaid: Int = penaltyDetails.lateSubmissionPenalty
    .map(
      _.details.count(point =>
        (point.chargeOutstandingAmount.isDefined && !point.chargeOutstandingAmount.contains(BigDecimal(0))) && (point.penaltyCategory.contains(
          LSPPenaltyCategoryEnum.Charge) || point.penaltyCategory.contains(LSPPenaltyCategoryEnum.Threshold))))
    .getOrElse(0)

  private val financialLSPs: Int = penaltyDetails.lateSubmissionPenalty
    .map(
      _.details.count(point =>
        (point.penaltyCategory.contains(LSPPenaltyCategoryEnum.Threshold) || point.penaltyCategory.contains(LSPPenaltyCategoryEnum.Charge))
          && !point.penaltyStatus.equals(LSPPenaltyStatusEnum.Inactive)))
  ...
  private val amountOfLSPsUnderAppeal: Int = penaltyDetails.lateSubmissionPenalty
    .map(_.details.count(point => point.appealInformation.exists(_.exists(_.appealStatus.contains(AppealStatusEnum.Under_Appeal)))))
    .getOrElse(0)

  private val numberOfPartiallyPaidLSPs: Int = penaltyDetails.lateSubmissionPenalty
    .map(
      _.details.count(point =>
  ...
  private val numberOfPaidLPPs: Int = penaltyDetails.latePaymentPenalty
    .flatMap(
      _.details.map(
        _.count(penalty =>
  ...
  private val numberOfUnpaidLPPs: Int = penaltyDetails.latePaymentPenalty
    .flatMap(
      _.details.map(
        _.count(penalty =>
  ...
  private val numberOfPartiallyPaidLPPs: Int = penaltyDetails.latePaymentPenalty
    .flatMap(
      _.details.map(
        _.count(penalty =>
  ...
  private val totalNumberOfLPPs: Int = penaltyDetails.latePaymentPenalty
    .flatMap(_.details.map(_.count(penalty => !penalty.appealInformation.exists(_.exists(_.appealStatus.contains(AppealStatusEnum.Upheld))))))
    .getOrElse(0)

  private val amountOfLPPsUnderAppeal: Int = penaltyDetails.latePaymentPenalty
    .flatMap(_.details.map(_.count(penalty => penalty.appealInformation.exists(_.exists(_.appealStatus.contains(AppealStatusEnum.Under_Appeal))))))
    .getOrElse(0)
  ...
  def getOptActiveTimeToPay: Option[TimeToPay] = {
    ...
    for {
      ...
      optActiveTimeToPay <- optSeqTimeToPay.find(penalty =>
        (penalty.TTPEndDate, penalty.TTPStartDate) match {
          case (Some(endDate), Some(startDate)) =>
            DateHelper.isDateAfterOrEqual(dateNow, startDate) && DateHelper.isDateBeforeOrEqual(dateNow, endDate)
