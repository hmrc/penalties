...
class APIController @Inject() (auditService: AuditService,
  ...
  private def handlePenaltyDetailsResponse(response: GetPenaltyDetailsResponse, agnosticEnrolmentKey: AgnosticEnrolmentKey)(implicit
      request: Request[_],
  ...
  private def callFinancialDetailsForManualLPPs(enrolmentKey: AgnosticEnrolmentKey)(implicit hc: HeaderCarrier): Future[Option[FinancialDetails]] =
    getFinancialDetailsService.getFinancialDetails(enrolmentKey, None).map { financialDetailsResponseWithoutClearedItems =>
  ...
  def getFinancialDetails(regime: Regime,
    ...
    val response = getFinancialDetailsService.getFinancialDetailsForAPI(
      enrolmentKey,
      searchType,
      searchItem,
      dateType,
  ...
  private def sendFinancialAudit(response: HttpResponse, enrolmentKey: AgnosticEnrolmentKey)(implicit
      hc: HeaderCarrier,
  ...
  def getPenaltyDetails(regime: Regime, idType: IdType, id: Id, dateLimit: Option[String]): Action[AnyContent] = authAction.async {
    implicit request =>
      ...
      hipPenaltyDetailsConnector.getPenaltyDetailsForAPI(enrolmentKey, dateLimit).map { response =>
  ...
  private def processSuccessResponse(response: HttpResponse, enrolmentKey: AgnosticEnrolmentKey): Result = {
    ...
    val validateBody: JsResult[GetPenaltyDetails] = convertHipResponseToGetPenaltyDetails(jsonBody)
    validateBody match {
  ...
  private def sendPenaltiesAudit(response: HttpResponse, enrolmentKey: AgnosticEnrolmentKey)(implicit
      hc: HeaderCarrier,
      request: Request[_]): Unit = {
    val jsonBody: JsValue = tryJsonParseOrJsString(response.body)
