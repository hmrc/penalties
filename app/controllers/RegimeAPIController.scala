...
class RegimeAPIController @Inject() (auditService: AuditService,
      ...
      _.fold(
        {
          ...
          case PenaltyDetailsParser.PenaltyDetailsFailureResponse(status, _) if status == UNPROCESSABLE_ENTITY =>
            ...
            val responsePayload = PenaltyDetailsSuccessResponse(
              PenaltyDetails(Instant.now(), totalisations = None, lateSubmissionPenalty = None, latePaymentPenalty = None, breathingSpace = None))
            logger.info(
              s"[RegimeAPIController][getSummaryDataForVRN] - 1812 call (VATVC/BTA API) returned $status for VRN: $id - Overriding response")
          ...
          case PenaltyDetailsParser.PenaltyDetailsMalformed =>
            ...
            logger.error(
              s"[RegimeAPIController][getSummaryDataForVRN] - 1812 call (VATVC/BTA API) returned invalid body - failed to parse penalty details response for VRN: $id")
  ...
  private def returnResponseForAPI(penaltyDetails: PenaltyDetails,
  ...
  def getFinancialDetails(regime: Regime,
    ...
    val response = getFinancialDetailsConnector.getFinancialDetailsForAPI(
      enrolmentKey,
      searchType,
      searchItem,
      dateType,
