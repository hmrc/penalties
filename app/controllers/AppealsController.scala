...
import services.AppealService.MultiplePenaltiesError
...
class AppealsController @Inject()(val appConfig: AppConfig,
  ...
  private def checkAndReturnResponseForPenaltyData(penaltyDetails: GetPenaltyDetails,
    ...
    val lspPenalty: Option[LSPDetails] = penaltyDetails.lateSubmissionPenalty.flatMap {
      _.details.find(_.penaltyNumber == penaltyIdToCheck)
    }
    val lppPenalty: Option[LPPDetails] = penaltyDetails.latePaymentPenalty.flatMap {
      _.details.flatMap(_.find(_.penaltyChargeReference.contains(penaltyIdToCheck)))
    }

    (appealType, lspPenalty, lppPenalty) match {
      case (Late_Submission, Some(penalty), _) =>
        ...
        buildLSPAppealData(penalty, appealType).fold {
          logger.error(s"[RegimeAppealsController][getAppealsData] Penalty ID: $penaltyIdToCheck for enrolment key: $enrolmentKey is missing required late-submission fields (lateSubmissions / tax-period dates) for $appealType.")
          InternalServerError("Penalty data is incomplete for the requested appeal.")
        }(data => Ok(Json.toJson(data)))
      case (Late_Payment | Additional, _, Some(penalty)) =>
        logger.info(s"[RegimeAppealsController][getAppealsData] Penalty ID: $penaltyIdToCheck for enrolment key: $enrolmentKey found in ETMP for $appealType.")
        Ok(Json.toJson(buildLPPAppealData(penalty, appealType)))
      case _ =>
  ...
  private def buildLSPAppealData(penalty: LSPDetails, appealType: AppealTypeEnum.Value): Option[AppealData] =
    for {
      submissions <- penalty.lateSubmissions
      earliest    <- PenaltyPeriodHelper.earliestSubmissionByPenaltyStartDate(submissions)
      startDate   <- earliest.taxPeriodStartDate
      endDate     <- earliest.taxPeriodEndDate
      dueDate     <- earliest.taxPeriodDueDate
    } yield AppealData(
      `type` = appealType,
      startDate = startDate,
      endDate = endDate,
      dueDate = dueDate,
      dateCommunicationSent = penalty.communicationsDate.getOrElse(appConfig.getTimeMachineDateTime.toLocalDate)
    )

  private def buildLPPAppealData(penalty: LPPDetails, appealType: AppealTypeEnum.Value): AppealData =
    AppealData(
      `type` = appealType,
      startDate = penalty.principalChargeBillingFrom,
      endDate = penalty.principalChargeBillingTo,
      dueDate = penalty.principalChargeDueDate,
      dateCommunicationSent = penalty.communicationsDate.getOrElse(appConfig.getTimeMachineDateTime.toLocalDate)
    )
  ...
  private def submitAppealToPEGA(appealSubmission: AppealSubmission,
                                 enrolmentKey: AgnosticEnrolmentKey,
                                 penaltyNumber: String,
                                 correlationId: String,
                                 isMultiAppeal: Boolean)
    ...
    appealService.submitAppeal(appealSubmission, enrolmentKey, penaltyNumber, correlationId).flatMap {
      case Left(error) =>
        Future.successful(pegaSubmissionFailed(error, enrolmentKey, penaltyNumber, correlationId))
      case Right(pegaResponse) =>
        handleSuccessfulPegaSubmission(appealSubmission.appealInformation, pegaResponse, enrolmentKey, penaltyNumber, correlationId, isMultiAppeal)
    }

  private def pegaSubmissionFailed(error: AppealsParser.ErrorResponse,
                                   enrolmentKey: AgnosticEnrolmentKey,
                                   penaltyNumber: String,
                                   correlationId: String): AppealSubmissionResponseModel = {
    logger.error(s"[RegimeAppealsController][submitAppeal] Error submitting appeal to PEGA for user with enrolment: $enrolmentKey penalty $penaltyNumber - Received error from PEGA with status ${error.status} and error message: ${error.body} " +
      s"for correlation ID: $correlationId")
    AppealSubmissionResponseModel(error = Some(error.body), status = error.status)
  }

  private def handleSuccessfulPegaSubmission(appeal: AppealInformation,
                                             pegaResponse: AppealResponseModel,
                                             enrolmentKey: AgnosticEnrolmentKey,
                                             penaltyNumber: String,
                                             correlationId: String,
                                             isMultiAppeal: Boolean)
    ...
    logger.info(s"[RegimeAppealsController][submitAppeal] Received caseID response: ${pegaResponse.caseID} from downstream.")

    val notificationsResult = appeal match {
      case other: OtherAppealInformation if other.uploadedFiles.isDefined =>
        appealService.createSDESNotifications(other.uploadedFiles, pegaResponse.caseID)
      case _ =>
        Right(Seq.empty)
    }
    ...
    notificationsResult match {
      case Left(error) =>
        logger.error(s"[RegimeAppealsController][submitAppeal] Unable to create file notifications for user with enrolment: $enrolmentKey penalty $penaltyNumber (correlation ID: $correlationId) - ${error.message}")
        Future.successful(returnErrorResponseIfMultiAppeal(isMultiAppeal)(
          s"Appeal submitted (case ID: ${pegaResponse.caseID}, correlation ID: $correlationId) but failed to create file notifications")(pegaResponse.caseID))

      case Right(notifications) if notifications.isEmpty =>
        Future.successful(AppealSubmissionResponseModel(caseId = Some(pegaResponse.caseID), status = OK))

      case Right(notifications) =>
        postNotificationsToOrchestrator(notifications, pegaResponse.caseID, enrolmentKey, penaltyNumber, correlationId, isMultiAppeal)
    }
  }

  private def postNotificationsToOrchestrator(notifications: Seq[SDESNotification],
                                              caseId: String,
                                              enrolmentKey: AgnosticEnrolmentKey,
                                              penaltyNumber: String,
                                              correlationId: String,
                                              isMultiAppeal: Boolean)
                                             (implicit hc: HeaderCarrier, request: Request[_]): Future[AppealSubmissionResponseModel] = {
    val redactedNotification = notifications.map(notification => notification.copy(file = notification.file.copy(location = "HIDDEN")))
    logger.info(s"[RegimeAppealsController][submitAppeal] Posting SDESNotifications: $redactedNotification to Orchestrator")

    fileNotificationOrchestratorConnector.postFileNotifications(notifications).map { response =>
      response.status match {
        ...
        case status =>
          ...
          auditStorageFailureOfFileNotifications(notifications)
          returnErrorResponseIfMultiAppeal(isMultiAppeal)(
            s"Appeal submitted (case ID: $caseId, correlation ID: $correlationId) but received $status response from file notification orchestrator")(caseId)
      }
    }.recover {
      case e =>
        ...
        auditStorageFailureOfFileNotifications(notifications)
        returnErrorResponseIfMultiAppeal(isMultiAppeal)(
          s"Appeal submitted (case ID: $caseId, correlation ID: $correlationId) but failed to store file uploads with unknown error")(caseId)
    }
  }

  private def handleMultiplePenaltiesResponse(multiplePenaltiesData: Either[MultiplePenaltiesError, Option[MultiplePenaltiesData]],
                                              enrolmentKey: AgnosticEnrolmentKey,
                                              penaltyId: String): Result = {
    multiplePenaltiesData.fold(
      error => {
        logger.error(s"[RegimeAppealsController][getMultiplePenaltyData] Unable to create multiple penalty data for enrolment: $enrolmentKey penalty ID: $penaltyId - ${error.message}")
        InternalServerError("Unable to create multiple penalty data.")
      },
      ...
      getPenaltyDetailsService.getPenaltyDetails(agnosticEnrolmentKey).map {
        _.fold(
          ...
          success => {
            ...
            val multiplePenaltiesData = appealService.findMultiplePenalties(penaltyDetails, penaltyId, regime.value)
            handleMultiplePenaltiesResponse(multiplePenaltiesData, agnosticEnrolmentKey, penaltyId)
          }
        )
      }
    }
